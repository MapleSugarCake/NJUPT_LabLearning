...
1. 首先显示版本号。
2. 第一次输入询问“是否自定义设置？默认请选N[y/N]：”。回车或 `n` 使用默认值：debug 关闭、课程线程数 4、强制重新提交关闭。
3. 选择 `y` 后，依次选择是否开启 debug、课程线程数和是否强制重新提交所有课程。线程数只接受 1–4，回车默认 4；强制重新提交输入 `y` 启用，回车或 `n` 关闭；非法输入重新询问。
...
7. 显示课程进度、失败原因及最终汇总。默认跳过已完成课程；强制模式会明确提示已启用，并重新处理全部课程，汇总跳过数为 0。

`y/n` 不区分大小写。本次设置不保存到磁盘。Token 仍通过隐藏输入获取。

“强制重新提交所有课程”只针对本轮首次课程列表接口返回的课程，沿用数据校验及按课程 ID 去重规则。无论原状态是否完成，每门课程均重新校验视频总时长、获取题目、按返回顺序逐题提交答案、上报完整视频时长、标记完成并回读核验。回读不会追加新的课程任务。此模式不删除历史记录，也不预设服务端会替换或保留旧记录；任一步失败即停止该课程后续写入，认证失效、取消、并发及重试规则保持不变。
...
- 每门课程先校验视频总时长，再按返回顺序逐题提交；全部成功后一次上报完整视频时长、标记完成，最后回读课程列表核验。无题课程同样执行视频步骤。
- 视频总时长取课程列表 `duration`，用 Decimal 保留精度；提交时截取整数秒字符串（例如 `123.456789` → `"123"`），通过 `finishRate` 的 `watchDuration` 一次提交，不每隔十秒发送。整数秒不得超过总时长，不向上取整增加不足一秒的尾部。成功浏览器抓包使用整数秒字符串；列表返回的同名 `watchDuration` 是百分比，不能作为上报秒数。缺失或无效总时长会让该课程失败，不猜测时长。
- 仅当 `isFinish` 表示完成且视频百分比恰好为 `100` 时，默认模式跳过课程；强制模式绕过此跳过判断。两种模式都必须在本轮回读达标后才能报告处理成功。保留大于 100 的实际百分比，不截成 100，也不当成未知。
- 首次 `finish` 成功后回读状态；若读取到有效百分比且不等于 100（包括超 100），最多追加一次 `finish`，然后再次回读核验。只补交完成请求，不重复答题或 `finishRate`；两次完成提交后仍不达标则报告失败及实际进度。
- 请求失败、超时、锁错误、认证失效、课程缺失、非法百分比或回读失败不会触发上述补交。回读 GET 仍遵守下述传输重试规则。
- 课程 GET 可以并行。所有答题、视频进度和课程完成 POST 共用一个写入锁，最大同时执行数为 1。
...
- 不自动刷新 Token、不在课程线程中重新登录、不重放失败的写入。上述第二次 `finish` 仅用于首次写入已确认成功、但状态核验显示进度不等于 100 的业务补交。
- debug 日志记录原始视频总秒数、实际上报整数秒数，以及脱敏且有界的业务错误 code/message，便于区分请求格式问题与服务端锁错误；不会记录完整请求体或原始错误页。锁错误不会触发自动重试。

浏览器抓包证明正常流程使用整数秒进度上报；使用者反馈超 100% 后再次提交 `finish` 可以恢复完成。本实现使用不超过总时长的整数秒，并进行最多一次完成补交；其实际服务端效果仍需维护者用本人账号在校外环境验收。验收应确认每门课程仅一次 `finishRate`、最多两次 `finish`，且刷新、重新进入后视频进度为 100% 并已完成；模拟测试不代表真实账号验证成功。
...
二次开发后，建议按以下顺序检查，并分别记录结果：

1. **运行自动测试**：使用上面的测试命令检查代码逻辑。测试使用模拟数据，不连接学校，也不使用真实账号。
2. **检查打包结果**：确认 Python 包可以安装和导入，EXE 可以正常启动、显示版本和退出。打包成功不代表登录或课程处理一定成功。
3. **手动检查实际效果**：使用本人账号，分别检查校园网和校外网络下的登录、课程列表及处理结果。账号密码登录和浏览器登录要分开验证；处理课程时检查每门课程的执行顺序、最多四门并发和所有 POST 串行。最后刷新或重新进入网页，确认视频进度为 100% 且课程显示完成。

验证校外登录时，要实际切换到校外网络并重新登录；只在校园网内选择“校外 VPN”选项，不算完成校外验证。

**只想测试登录时**，单独调用 `njupt_auth.authenticate`，用 `with` 管理返回的认证结果。该接口会自动检查主会话的访问权限；如需检查课程线程使用的独立会话，可通过 `auth.session_factory()` 创建，再调用 `check_access`，并用 `with` 关闭会话。不要直接运行完整 CLI 来测试登录，它会继续处理课程。接口和资源管理约定见 [AGENTS.md](AGENTS.md)。

真实账号信息只在本机交互输入，不写入代码、命令或环境变量，不提交到仓库。密码会明文显示，测试时避免录屏或截图。提交修改时说明测试了哪些网络和登录方式，哪些通过、哪些失败；没有测过的写“未验证”，不要把模拟测试通过当成真实环境可用。
